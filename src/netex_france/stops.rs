    ...
    fn generate_centroid(&self, coord: &'a Coord) -> Option<Element> {
        if *coord != Coord::default() {
            if let Ok(coord_epsg2154) = self.converter.convert(*coord) {
                let lat = Element::builder("Latitude").append(Node::Text(format!("{}", coord.lat))).build();
                let lon = Element::builder("Longitude").append(Node::Text(format!("{}", coord.lon))).build();
                ...
                let pos = Element::builder("gml:pos")
                    ...
                    .append(coord_text)
                    .build();
                let location = Element::builder("Location").append(lat).append(lon).append(pos).build();
